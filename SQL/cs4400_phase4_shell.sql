...
-- Bank Management Project - Phase 4
...
begin
  ...
  -- query success table update for part IV
    declare curr_time timestamp;
    set curr_time = current_timestamp();
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: create corporation procedure failed", "create_corporation", curr_time);

	if ip_corpID not in (select corpID from corporation)
		and ip_shortName not in (select shortName from corporation)
        and ip_longName not in (select longName from corporation)
    then
		insert into corporation values (ip_corpID, ip_shortName, ip_longName, cast(ip_resAssets as signed));
         -- update the success table because the procedure succeeded.
        update success
	    set procedure_status = "succeeded"
	    where procedure_timestamp = curr_time;

	    update success
	    set procedure_message = null
	    where procedure_timestamp = curr_time;
	end if;
end //
delimiter ;
...
begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: create bank procedure failed", "create_bank", curr_time);
...
sp_3: begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: start employee role procedure failed", "start_employee_role", curr_time);
...
sp_4: begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: start customer role procedure failed", "start_customer_role", curr_time);
...
sp_5: begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: stop employee role procedure failed", "stop_employee_role", curr_time);
...
sp_6:begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: stop customer role procedure failed", "stop_customer_role", curr_time);
...
begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: hire worker procedure failed", "hire_worker", curr_time);
...
begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: replace manager procedure failed", "replace_manager", curr_time);


	if ip_perID in (select perID from employee) and ip_perID not in (select manager from bank) and ip_perID not in (select perID from workfor) and ip_salary >= 0
	then 
...
sp_9:begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: add account access procedure failed", "add_account_access", curr_time);

	-- Implement your code here
    IF ip_bankID NOT IN (SELECT bankID FROM bank) THEN LEAVE sp_9; END IF;
 -- verifies that account exists
    IF (
        ...
        -- don't double-add access if that person already has access
        AND ((ip_bankID, ip_accountID) NOT IN (SELECT bankID, accountID FROM access WHERE perID = ip_customer))
	) THEN -- add access:
...
-- query success table update for part IV
 ...
 -- count how many customers have access to the account
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: remove account access procedure failed", "remove_account_access", curr_time);
...
sp_11:begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: fee creation procedure failed", "create_fee", curr_time);
    
    
if (ip_bankID, ip_accountID, ip_fee_type) in (select bankID, accountID, fee from interest_bearing_fees) then leave sp_11; end if;
...
begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: start overdraft procedure failed", "start_overdraft", curr_time);
...
-- query success table update for part IV
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: stop overdraft procedure failed", "stop_overdraft", curr_time);
    
 IF (-- check to see that the checking account exists
		(ip_checking_accountID IN (SELECT accountID from checking)) 
		AND 
        -- check to see if the requestor has access to the checking account 
...
sp_14:begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: account deposit procedure failed", "account_deposit", curr_time);
    ...
    if (ip_requester not in (select perID from access where (ip_bankID, ip_accountID) = (bankID, accountID)))
		or ip_deposit_amount < 0
	then leave sp_14; end if;
	

	update bank_account set
		balance = ifnull(balance, 0) + ip_deposit_amount where (ip_bankID, ip_accountID) = (bankID, accountID);
	update access set
			dtAction = ip_dtAction where (ip_requester, ip_bankID, ip_accountID) = (perID, bankID, accountID);
        
	update interest_bearing set
		dtDeposit = ip_dtAction where (ip_bankID, ip_accountID) = (bankID, accountID);
...
sp_15:begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: account withdrawal procedure failed", "account_withdrawal", curr_time);
...
sp_16:begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: account transfer procedure failed", "account_transfer", curr_time);
    ...
    if exists (select * from checking where (bankID, accountID) = (ip_from_bankID, ip_from_accountID)) then
        ...
        -- if there is no protection account for checking account
        if isnull((select protectionAccount from checking where (bankID, accountID) = (ip_from_bankID, ip_from_accountID))) then leave sp_16; end if;
        ...
        if overdraft_needed <= protecting_account_bal then
			...
			update access set
				dtAction = ip_dtAction where (bankID, accountID) = (ip_from_bankID, ip_from_accountID);
			update checking set
				amount = overdraft_needed where (bankID, accountID) = (ip_from_bankID, ip_from_accountID);
            update checking set
...
begin
 ...
 -- query success table update for part IV
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: pay employees procedure failed", "pay_employees", curr_time);
...
begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: penalize accounts procedure failed", "penalize_accounts", curr_time);
...
begin
    ...
    delete from success;
    insert into success (procedure_status, procedure_message, procedure_id, procedure_timestamp)
		values ("failed", "Error: accrue interest procedure failed", "accrue_interest", curr_time);
    ...
    join savings on (bank_account.bankID, bank_account.accountID) = (savings.bankID, savings.accountID)
		and ifnull(balance, 0) >= ifnull(minBalance, 0)
	set balance = truncate(ifnull(balance, 0) * (1 + ifnull(interest_rate, 0) / 100), 0);
